...
const LeaveRequestHistory = () => {
    // Removed employeeId from useParams as it's now a general view
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [searchTerm, setSearchTerm] = useState('');
    const [filterEmployeeId, setFilterEmployeeId] = useState('all'); // New state for employee filter
    ...
    // This list will now be used for the employee filter dropdown
    ...
    const filteredRequests = useMemo(() => {
        ...
        // Filter by employeeId if a specific employee is selected in the filter dropdown
        if (filterEmployeeId !== 'all') {
            filtered = filtered.filter(req => req.employee.id === filterEmployeeId);
        }
    ...
    }, [leaveRequests, searchTerm, filterEmployeeId, filterStatus, filterLeaveType, filterYear]);
    ...
    const clearFilters = () => {
        setSearchTerm('');
        setFilterEmployeeId('all'); // Reset employee filter
        setFilterStatus('all');
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-900 dark:to-black text-gray-900 dark:text-gray-100 p-4 sm:p-6 lg:p-8 transition-colors duration-300 font-inter">
            ...
            {!loading && ( // Only render content if not loading
                <>
                    ...
                    <Card className="p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 mb-10">
                        <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-100 flex items-center gap-3">
                            <Filter size={28} className="text-teal-600 dark:text-teal-400" /> Filter Leave Requests
                        </h2>
